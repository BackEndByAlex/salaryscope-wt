...
1. The click's `lngLat` is sent to BigDataCloud's reverse geocode API — a free, no-API-key, CORS-enabled endpoint that returns `{ city, countryName }` for any coordinate
