...
A dual-thumb slider built from scratch using pointer events. The range is derived from the loaded records — min and max are computed from `salaryInUsd ?? salary` so all records are covered including non-USD ones. Dragging either thumb filters the visible list client-side without a new API call.
...
Breakdown stats at the top of the right panel — shown immediately when a country or city is selected, before the user scrolls to the salary list.

Fires `COUNTRY_SIDEBAR_QUERY` or `CITY_SIDEBAR_QUERY` for the selected location. Uses GraphQL aliases to fetch eight counts in a single request:
...
A simple wrapper with a small uppercase label above its children. Used to visually group content inside the sidebar — the salary records list is wrapped in one of these. No logic.
