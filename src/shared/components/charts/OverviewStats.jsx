const STATS = [
  { label: "Salary Records",  icon: "database",       valueKey: "records",    color: "text-on-surface" },
  { label: "Countries",       icon: "public",         valueKey: "countries",  color: "text-primary" },
  { label: "Companies",       icon: "corporate_fare", valueKey: "companies",  color: "text-secondary" },
  { label: "Job Categories",  icon: "work",           valueKey: "categories", color: "text-tertiary" },
]
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-px bg-outline-variant/10">
      {STATS.map(({ label, icon, valueKey, color }) => (
        <div key={label} className="bg-surface-container p-6 flex flex-col gap-3">
          <span className={`material-symbols-outlined text-2xl ${color} opacity-70`}>{icon}</span>
