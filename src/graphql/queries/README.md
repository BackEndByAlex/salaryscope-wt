...
Used by `DashboardFilterSidebar` to show only the values that actually exist in the data for the selected region. Scoped queries mean the user never sees filter options that would return zero results.
